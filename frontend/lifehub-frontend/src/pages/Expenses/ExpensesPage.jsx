...
export default function ExpensesPage({ isDarkMode, groupData }) {
  ...
  const [expenses, setExpenses] = useState([
    { id: 1, category: '餐飲', item: '全聯買菜', amount: 1200, payer: '小明', date: '2026-03-25', participants: ['小明', '小華', '小強'] },
    { id: 2, category: '生活家居', item: '衛生紙、洗衣精', amount: 450, payer: '小華', date: '2026-03-24', participants: ['小明', '小華'] },
  ]);
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={cn(
                  ...
                  <th className="p-6 text-right">參與成員</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={e.id} className={cn(
                      ...
                      <td className="p-6 text-right">
                        ...
                        <p className="mt-1 text-[10px] text-slate-400 font-bold">{e.participants.length} 人參與</p>
                      </td>
                    </tr>
                )})}
              </tbody>
          ...
          <div className={cn(
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-slate-500">本月支出進度</span>
              ...
              <div className={cn(
                ...
                <p className="text-xs text-slate-500 font-bold mb-1 uppercase tracking-wider">目前個人總計</p>
          ...
          <div className={cn(
            ...
            <div className="p-8 pb-10">
              <div className="flex justify-between items-center mb-8">
                <h2 className={cn("text-2xl font-black", { "text-white": isDarkMode, "text-slate-900": !isDarkMode })}>記一筆新支出</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">項目名稱與金額內容</label>
                ...
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">支付成員</label>
                  <select 
                    className={cn(
                      "w-full p-4 rounded-2xl border font-bold text-sm focus:ring-2 focus:ring-blue-500 transition outline-none",
                      { "bg-slate-800 border-slate-700 text-white": isDarkMode, "bg-slate-50 border-slate-100 text-slate-900": !isDarkMode }
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">參與分攤成員 (點擊切換)</label>
                    <button 
                      type="button"
                      onClick={handleSelectAll}
                      className="text-xs font-bold text-blue-600 hover:text-blue-700"
                    >
                      {formData.selectedMembers.length === groupData?.members.length ? '取消全選' : '全選'}
                    </button>
                  </div>
