...
const AuditLogList = ({ isDarkMode }) => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center p-20 space-y-4">
      ...
      <p className="font-bold text-slate-400">正在提取日誌紀錄...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h2 className={`text-2xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'} tracking-tight`}>操作日誌</h2>
        <p className="text-slate-500 font-medium text-sm">追蹤系統內的所有變更紀錄</p>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className={isDarkMode ? 'bg-slate-800/30' : 'bg-slate-50/50'}>
                <th className="premium-table-header w-[220px]">時間點</th>
                <th className="premium-table-header w-[150px]">執行人員</th>
                <th className="premium-table-header w-[120px]">動作類別</th>
                <th className="premium-table-header w-[150px]">目標模組</th>
                <th className="premium-table-header">異動詳細摘要</th>
              </tr>
            </thead>
