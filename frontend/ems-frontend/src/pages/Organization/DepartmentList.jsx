...
import { Briefcase, Plus, Search, Trash2, Edit3, ShieldCheck, Hash } from 'lucide-react';
import employeeApi from '../../api/employeeApi';
import OrganizationModal from '../../components/OrganizationModal';

const DepartmentList = ({ isDarkMode }) => {
  ...
  const handleDelete = async (id, name) => {
    if (window.confirm(`確定要刪除部門「${name}」嗎？這將不會刪除該部門下的員工，但會取消他們的關聯。`)) {
      try {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex justify-between items-end">
        <div>
          <h2 className={`text-2xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'} tracking-tight`}>部門管理</h2>
          <p className="text-slate-500 dark:text-slate-400 font-medium">管理企業組織架構與部門描述</p>
        </div>
        <button 
          onClick={handleAdd}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {departments.map((dep) => (
          <div key={dep.id} className="premium-card p-8 group relative overflow-hidden">
             {/* Gradient Accent */}
            <div className={`absolute top-0 right-0 w-24 h-24 ${isDarkMode ? 'bg-blue-900/10' : 'bg-blue-50'} rounded-full -mr-12 -mt-12 transition-transform duration-500 group-hover:scale-150`} />
             ...
             <div className="relative">
              <div className={`w-14 h-14 ${isDarkMode ? 'bg-slate-800 border-slate-700 text-blue-400' : 'bg-blue-50 border-blue-100 text-blue-600'} border rounded-3xl flex items-center justify-center shadow-sm mb-6 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300`}>
                <ShieldCheck size={28} strokeWidth={2.5} />
              </div>
              
              <h3 className={`text-xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'} mb-2`}>{dep.name}</h3>
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400 line-clamp-2 min-h-[2.5rem]">
                {dep.description || '尚無描述，點擊編輯以新增。'}
              </p>
        ...
        >
          ...
          <span className="font-black text-lg">新增更多部門...</span>
        </div>
      </div>
