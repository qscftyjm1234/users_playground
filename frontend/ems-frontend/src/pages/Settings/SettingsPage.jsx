...
export default function SettingsPage({ isDarkMode }) {
  const [profile, setProfile] = useState({ username: '王小明', email: 'xiaoming@example.com' });
  ...
  return (
    <div className={`p-6 rounded-3xl shadow-sm border ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'} max-w-4xl`}>
      <h2 className={`text-2xl font-black mb-6 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>後台控制 / 個人設定</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className={`block text-sm font-bold mb-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>電子信箱</label>
              <Input 
                value={profile.email} 
                onChange={e => setProfile({...profile, email: e.target.value})}
                disabled
              />
              <p className="text-xs text-slate-500 mt-2">電子信箱無法更改，若需修改請聯絡系統管理員。</p>
            </div>
