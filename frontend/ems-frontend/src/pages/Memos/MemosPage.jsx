...
export default function MemosPage({ isDarkMode, groupData }) {
  ...
  const [memos, setMemos] = useState([
    { id: 1, content: '週五晚上一起看 Netflix 新上的電影！別忘了買爆米花🍿', tags: ['休閒', '電影'], color: 'bg-amber-100' },
    { id: 2, content: 'WiFi 密碼：12345678\n大門密碼：8888', tags: ['重要資料'], color: 'bg-blue-100' },
    { id: 3, content: '想買一台新的微波爐，大家覺得 Panasonic 的好嗎？', tags: ['購物討論'], color: 'bg-emerald-100' },
    { id: 4, content: '房東來收租時間改為每個月 5 號。', tags: ['租屋資訊'], color: 'bg-rose-100' }
  ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className={cn("text-2xl font-black", { "text-white": isDarkMode, "text-slate-900": !isDarkMode })}>靈感與備忘錄</h2>
          <p className="text-sm text-slate-500 font-medium">{groupData?.name} 的秘密筆記本</p>
        </div>
        <button 
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        <div className="flex-1 max-w-sm">
          <Input placeholder="搜尋內容..." prefix={<Search size={18} className="text-slate-400" />} />
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={cn(
            ...
            <div className="flex justify-between items-center mb-8">
              <h2 className={cn("text-2xl font-black", { "text-white": isDarkMode, "text-slate-900": !isDarkMode })}>新增備忘錄</h2>
            ...
            <form onSubmit={handleAddMemo} className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-3">選擇顏色</label>
