...
export default function Sidebar({ isDarkMode, setIsDarkMode, activeGroup, isGroupContext }) {
  ...
  return (
    <aside className={cn(
      ...
      <nav className="flex flex-col gap-2 overflow-y-auto pr-2 custom-scrollbar">
        ...
        {isGroupContext && (
          <>
            ...
            <SidebarItem icon={WalletCards} label="支出分攤" active={isExpenses} onClick={() => navigate(`/groups/${activeGroupId}/expenses`)} isDarkMode={isDarkMode} />
            <SidebarItem icon={ListTodo} label="家務分配" active={isTasks} onClick={() => navigate(`/groups/${activeGroupId}/tasks`)} isDarkMode={isDarkMode} />
            <SidebarItem icon={CalendarDays} label="共同行程" active={isCalendar} onClick={() => navigate(`/groups/${activeGroupId}/calendar`)} isDarkMode={isDarkMode} />
        ...
        <SidebarItem icon={Settings} label="個人後台設定" active={isSettings} onClick={() => navigate('/settings')} isDarkMode={isDarkMode} />
        <SidebarItem icon={ShieldCheck} label="商業管理後台" active={isAdmin} onClick={() => navigate('/admin')} isDarkMode={isDarkMode} />
      </nav>
      ...
      <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
        ...
        <SidebarItem icon={LogOut} label="登出" isDarkMode={isDarkMode} />
      </div>
    </aside>
  );
}
